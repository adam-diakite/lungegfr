...
def process_patient(root, ct_png_folder, pet_png_folder, seg_nii_folder, patient_output_folder):
    ...
    for i, (ct_file, pet_file, seg_file) in enumerate(
            tqdm(zip(ct_files[:num_files], pet_files[:num_files], seg_files[:num_files]), total=num_files,
                 desc=f"Processing {patient_name}")):
...
def process_patient_area(root, seg_nii_folder, patient_output_folder):
    ...
    with open(csv_file_path, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['Slice Number', 'Tumor Area'])
        for i, areas in enumerate(tumor_areas_per_slice): # 40
            for area in areas:
