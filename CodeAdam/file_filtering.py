...
source_directory = "/media/adamdiakite/LaCie/Capricorn_immuno/Patients"  # Replace with the path to the source directory
destination_directory = "/media/adamdiakite/LaCie/CT-TEP_ICI"  # Replace with the path to the destination directory
...
def move_patients_with_subfolders(source_directory, destination_directory, subfolder_name1, subfolder_name2):
    """
    Moves patient folders and their contents to the specified destination directory if they contain both
    subfolders with names specified in 'subfolder_name1' and 'subfolder_name2'.
    :param source_directory: The source directory containing the patient folders.
    :param destination_directory: The destination directory where the patient folders will be moved.
    :param subfolder_name1: The name of the first subfolder to check for.
    :param subfolder_name2: The name of the second subfolder to check for.
    """
    # List all patient folders in the source directory
    patient_folders = [folder for folder in os.listdir(source_directory) if
                       os.path.isdir(os.path.join(source_directory, folder))]

    # Check each patient folder for the specified subfolders
    for folder in patient_folders:
        folder_path = os.path.join(source_directory, folder)
        contains_subfolder1 = os.path.exists(os.path.join(folder_path, subfolder_name1))
        contains_subfolder2 = os.path.exists(os.path.join(folder_path, subfolder_name2))

        if contains_subfolder1 and contains_subfolder2:
            destination_path = os.path.join(destination_directory, folder)
            shutil.move(folder_path, destination_path)

    print("Patient folders with both specified subfolders moved successfully.")
...
def delete_files_with_characters(folder_path, characters):
    """
    Deletes files in the given folder path and its subdirectories that have the same name as the specified characters.
    :param folder_path: Path to the folder.
    :param characters: String of characters to match in file names.
    """
    for root, dirs, files in os.walk(folder_path):
        for file_name in files:
            if file_name == characters:
                file_path = os.path.join(root, file_name)
                os.remove(file_path)

    print("Deletion of files completed successfully.")
copy_segmentation_file("/media/adamdiakite/LaCie/to treat","/media/adamdiakite/LaCie/dest_treat")